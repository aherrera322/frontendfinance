...
// Function to convert Excel date number to MySQL date
...
try {
    ...
    // Map column indices based on actual headers
    $columnMap = [];
    foreach ($headers as $index => $header) {
        ...
        switch ($header) {
            ...
            case 'RESERVATION DATE':
                $columnMap['res_date'] = $index;
                break;
            case 'APPROVE DATE':
            case 'APPROVAL DATE':
                $columnMap['approve_date'] = $index;
                break;
            case 'RES NUMBER':
            case 'RESERVATION NUMBER':
                $columnMap['res_number'] = $index;
                break;
            case 'RES TYPE':
            case 'RESERVATION TYPE':
                $columnMap['res_type'] = $index;
                break;
            case 'PREPAY':
            case 'PREPAYMENT':
                $columnMap['prepay'] = $index;
                break;
            case 'AGENCY':
            case 'AGENCY NAME':
                $columnMap['agency'] = $index;
                break;
            case 'PAY MODE':
            case 'PAYMENT MODE':
                $columnMap['pay_mode'] = $index;
                break;
            case 'API VALUE':
            case 'API':
                $columnMap['api_value'] = $index;
                break;
            case 'CREDIT':
    ...
    foreach ($rows as $index => $row) {
        try {
            ...
            $maxColumns = max(array_values($columnMap)) + 1;
            $row = array_pad($row, $maxColumns, '');
            
            // Extract data using column mapping
            $resDateRaw = isset($columnMap['res_date']) && !empty($row[$columnMap['res_date']]) ? $row[$columnMap['res_date']] : null;
            $resDate = $resDateRaw ? excelToDate($resDateRaw) : null;
            
            $agency = isset($columnMap['agency']) && !empty($row[$columnMap['agency']]) ? trim($row[$columnMap['agency']]) : null;
            $payMode = isset($columnMap['pay_mode']) && !empty($row[$columnMap['pay_mode']]) ? trim($row[$columnMap['pay_mode']]) : null;
            $apiValue = isset($columnMap['api_value']) && !empty($row[$columnMap['api_value']]) && is_numeric($row[$columnMap['api_value']]) ? (float)$row[$columnMap['api_value']] : null;
            $credit = isset($columnMap['credit']) && !empty($row[$columnMap['credit']]) && is_numeric($row[$columnMap['credit']]) ? (float)$row[$columnMap['credit']] : null;
            $cpc = isset($columnMap['cpc']) && !empty($row[$columnMap['cpc']]) ? trim($row[$columnMap['cpc']]) : null;
            $prepay = isset($columnMap['prepay']) && !empty($row[$columnMap['prepay']]) ? trim($row[$columnMap['prepay']]) : null;
            $status = isset($columnMap['status']) && !empty($row[$columnMap['status']]) ? trim($row[$columnMap['status']]) : null;
